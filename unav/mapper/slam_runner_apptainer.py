...
def run_stella_vslam_dense(
    ...
    shell_command = (
        ...
        f'/stella_vslam_examples/build/run_video_slam '
    ...
    # Run stella_vslam in the Singularity/Apptainer container
    cmd = [
        #"source", "~/bash_local_unav_server", "&&",
        "apptainer", "exec", "--nv", 
        "--bind", f"{host_data_root}:{container_data_root}",
        "--ipc", "--env", "DEBIAN_FRONTEND=noninteractive",
        f"../../images/{container_name}",
        "/bin/bash",
        "-c", shell_command
    ]
