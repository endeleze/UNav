from core.third_party.SuperPoint_SuperGlue.base_model import (
    dynamic_load,
)
from core.third_party.SuperPoint_SuperGlue import (
    extractors,
    matchers,
)
        ...
        for name, content in self.configs.items():
            if name == "superpoint+superglue":
                superpoint = Superpoint(
                    self.device, self.configs["superpoint+superglue"]
                )
                return superpoint.extract_local_features
            elif name == "superpoint+lightglue":
                superpoint = Superpoint(
                    self.device, self.configs["superpoint+lightglue"]
                )
                return superpoint.extract_local_features
            elif name == "sift":
                pass
