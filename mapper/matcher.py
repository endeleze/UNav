...
from core.colmap.utils_pose import load_colmap_images_file
from core.feature_filter import match_query_to_database, ransac_filter
from mapper.tools.matcher.geometry import fast_find_adjacent_and_pose_pairs
...
def compute_similarity_and_generate_topk_pairs(
    ...
    """
    Compute Top-K most similar pairs based on global descriptor similarity.
    """
...
def generate_and_stream_colmap(
    ...
    """
    End-to-end pipeline: generate candidate pairs -> LightGlue matching -> RANSAC verification -> 
    stream writing to COLMAP local features and matches files.
    Returns a list of verified image pairs.
    """
    ...
    # Generate direct adjacency pairs and global retrieval pairs
    direct = fast_find_adjacent_and_pose_pairs(
        img_names,
        poses,
    ...
    retrieval = compute_similarity_and_generate_topk_pairs(
        feat_cfg['global_feat_save_path'], top_k=matcher_config["top_k_matches"]
    )
    all_pairs = list({(min(a, b), max(a, b)) for a, b in set(direct) | set(retrieval)})

    # Group pairs by img1 for batch processing
    ...
    with h5py.File(feat_cfg['local_feat_save_path'], 'r') as f_local, \
        ...
        for img1, img2_list in progress:
            ...
            names2, p0_idx, p1_idx, scores_list = match_query_to_database(
                feat1, db_feats, img2_list, matcher, device, feature_score_threshold=matcher_config["feature_score_threshold"], threshold=matcher_config["min_keypoints"]
            )
            if not names2:
                skipped += 1
            ...
            valid_mask, in0, in1, inlier_scores = ransac_filter(
                p0_idx, p1_idx, k0_list, k1_list, scores_list, device, threshold=matcher_config["min_keypoints"]
            )

            # Stream writing verified matches and features to files
