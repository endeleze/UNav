...
def generate_commands(
    ...
    """
    Generate clock-face-style navigation commands based on direction and distance.

    Args:
        path_coords: List of (x, y) coordinates forming the path.
        initial_heading: Initial heading in degrees.
        scale: Meters per pixel scaling factor.

    Returns:
        List of text commands like "Turn left to 3 o'clock" and "Forward 2.3 m".
    """
    commands: List[str] = []
    ...
    for (x0, y0), (x1, y1) in zip(path_coords, path_coords[1:]):
        dx = x1 - x0
        dy = y0 - y1  # Invert y due to screen coordinate system
...
def convert_distance(meters: float, unit: Literal["meter", "feet"] = "meter") -> str:
    """
    Convert meters to the target unit and apply rounding convention.
    ...
    if unit == "feet":
        ...
        if meters < 1:
            ...
            rounded = round(meters * 2) / 2  # Round to nearest 0.5
...
def commands_from_result(
    ...
    """
    Generate spoken-style commands based on navigation path result.

    Args:
        navigator: FacilityNavigator instance.
        path_result: Output dictionary from path planner.
        initial_heading: Heading at start in degrees.
        unit: "meter" (default, with rounding) or "feet" (rounded to int).

    Returns:
        List of step-by-step text instructions.
    """
    ...
    # 1. Initial announcement of the starting room
    ...
    for i in range(len(coords) - 1):
        ...
        if key0.count("__") == 2 and key1.count("__") == 2:
            ...
            if (b0, f0) != (b1, f1):
                ...
                dyl = start_pt[1] - end_pt[1]  # screen y-inversion
        ...
        # --- E. Finalize forward instruction ---
...
def split_path_by_floor(
    ...
    Args:
        path_keys: List of node keys including "VIRT" and node IDs.
        path_coords: Corresponding list of (x, y) coordinates.

    Returns:
        A dictionary mapping "Building__Floor" to coordinates on that floor.
        The initial pose is included only in the first real floor.
    """
